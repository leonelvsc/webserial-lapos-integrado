...
export abstract class Command {
  ...
  protected constructor(
    protected command: string[3],
    protected data: Uint8Array
  ) {
  }

  protected static convertToByte(params: any): number[] {
    const buffer = [];

    const values: any[] = (typeof params === 'object') ? Object.values(params) : params;
  ...
  static convertToString(bytes: Uint8Array | number[]): string {
    let string = '';
  ...
  build(): Uint8Array {
    ...
    const command: any = new Uint8Array(buffer);
  ...
  protected checkSum(command: Uint8Array) {
    let cs = 0;
  ...
  protected checkSumVerify(command: Uint8Array) {
    let cs = 0;
